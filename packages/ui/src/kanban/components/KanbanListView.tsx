...
export function KanbanListView({
  ...
  const grouped = React.useMemo(() => {
    // Match board columns: a child is rendered ONLY as a row inside its parent's
    // card, never as a standalone row here. OWNER DECISION — the relationship is
    // the only place a child appears on the board, and the list view mirrors the
    // board. A child's verify verdict shows on its child row in the board's tree.
        ...
        : STATUS_ORDER.map((status) => {
            ...
            return (
              <div key={status} style={{ marginBottom: 24 }}>
                ...
                <div
                  style={{
                    display: collapsed[status] ? "none" : "flex",
...
function ListRow({
  ...
  return (
    ...
    >
      {/* Type icon */}
      <span style={{ fontSize: 12, textAlign: "center", flexShrink: 0 }}>
        {typeIcon}
      </span>
      ...
      <button
        ...
        style={{
          ...
          fontSize: 11,
          color:
            commentCount > 0 ? "var(--t-text-muted)" : "var(--t-border-faint)",
          background: "none",
          border: "none",
          cursor: "pointer",
          padding: "0 2px",
