...
function PasteHintBanner({
  storageKey = PASTE_HINT_KEY,
}: {
  storageKey?: string;
}) {
  ...
  return (
    ...
    >
      <span style={{ flex: 1, lineHeight: 1.4 }}>
        💡 <strong>Tip:</strong> Paste screenshots or files anywhere in this
        panel — they attach automatically.
      </span>
      <button
        onClick={dismiss}
        title="Dismiss"
...
export function DetailPanel({
  ...
  const [showCommentSendConfirm, setShowCommentSendConfirm] =
    React.useState(false);
  const [branchName, setBranchName] = React.useState("");
  const [showBranchPrompt, setShowBranchPrompt] = React.useState(false);
  const pendingReviewPatchRef = React.useRef<{
    taskId: string;
    patch: Partial<Task>;
  } | null>(null);
  ...
  React.useEffect(() => {
    showCommentSendConfirmRef.current = showCommentSendConfirm;
  ...
  React.useEffect(() => {
    isResizingRef.current = isResizing ?? false;
  }, [isResizing]);
  ...
  React.useEffect(() => {
    ...
    function handleEsc(e: KeyboardEvent) {
      ...
      if (showCommentSendConfirmRef.current) {
        setShowCommentSendConfirm(false);
        e.stopPropagation();
        return;
      }
      if (commentInputRef.current.trim().length > 0) {
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      trimmedDesc !== originalDescRef.current
    ) {
      setTitle(trimmedTitle);
      setDescription(trimmedDesc);
      originalTitleRef.current = trimmedTitle;
  ...
  async function loadBadgeCounts(taskId: string) {
    try {
      ...
      /* network error — badge counts stay stale */
    }
  }
  ...
  function handleApplyBranchAndReview() {
    ...
    const patch: Partial<Task> = {
      ...pendingReviewPatchRef.current.patch,
    ...
    <>
      <aside id="detail-panel" className="open" ref={panelRef}>
        {remoteLocked && (
          ...
          >
            🔒 Locked by <strong>{lockedByUser}</strong> — editing disabled
            until they finish.
          </div>
        )}
        {/* Header */}
        <div className="dp-header">
          ...
          >
            {onGoBack && (
              <button
                ...
                onMouseOver={(e) => {
                  e.currentTarget.style.background = "var(--p-border)";
                  e.currentTarget.style.color = "var(--p-blue-300)";
                  e.currentTarget.style.borderColor = "var(--p-blue)";
                }}
                onMouseOut={(e) => {
            ...
            <TypePicker
              ...
              onChange={(newType) => {
                if (!isAdd && task && newType !== type)
                  recordChange("type", type, newType);
                setType(newType);
            ...
            <input
              ...
              onFocus={(e) => {
                e.currentTarget.style.borderColor = "var(--p-blue)";
                e.currentTarget.style.background = "var(--p-input)";
              }}
              onBlur={(e) => {
            ...
            <button
              ...
              onMouseOver={(e) => {
                e.currentTarget.style.background = "var(--p-border)";
                e.currentTarget.style.color = "var(--p-text-m)";
              }}
              onMouseOut={(e) => {
                e.currentTarget.style.background = "transparent";
                e.currentTarget.style.color = "var(--p-text-g)";
          ...
          >
            ...
            {STATUS_BUTTONS.map((s) => (
              <button
                key={s.id}
                className={activeStatusClass(s.id)}
                data-status={s.id}
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && branchName.trim())
                    handleApplyBranchAndReview();
                  if (e.key === "Escape") handleCancelBranchPrompt();
                }}
                autoFocus
                style={{
                  flex: 1,
        ...
        {!isAdd && (
          <div className="dp-tabs" id="dp-tabs-row">
            <button
              className={`dp-tab${activeTab === "details" ? " active" : ""}`}
              id="dp-tab-details"
            ...
            <button
              className={`dp-tab${activeTab === "comments" ? " active" : ""}`}
              id="dp-tab-activity"
            ...
            >
              ...
              {commentCount > 0 && (
                <span
                  ...
                  style={{ color: "var(--p-text-g)", fontSize: 10 }}
                >
                  ({commentCount})
                </span>
              )}
            ...
            <button
              className={`dp-tab${activeTab === "files" ? " active" : ""}`}
              id="dp-tab-files"
        ...
        <div className="dp-body">
          ...
          <div
            className="dp-pane"
            id="dp-details-pane"
            style={{ display: !task || activeTab === "details" ? "" : "none" }}
          >
            <div
              style={
                remoteLocked
                  ? { pointerEvents: "none", opacity: 0.6, flexShrink: 0 }
                  : { flexShrink: 0 }
              }
            >
              <TaskDetailsTab
                ...
                originalDescription={
                  isAdd ? undefined : originalDescRef.current
                }
                onPriorityChange={
                  isAdd
                    ? undefined
                    : (p) => {
                        if (task) {
                          if (p !== priority)
                            recordChange("priority", priority || "—", p || "—");
                          onPatch(task.id, { priority: p || undefined });
                        }
                      }
                }
                githubUrl={githubUrl}
          ...
          <div
            ...
            id="dp-activity-pane"
            style={{
              display: activeTab === "comments" && !isAdd ? "" : "none",
            }}
          >
            ...
            <FilesList
              ...
              onPreview={(f) =>
                onFilePreview(
                  f.name,
                  f.linkedPath
                    ? `${baseUrl}/api/tasks/${task?.id}/files/${encodeURIComponent(f.name)}`
                    : `${baseUrl}${f.url}`,
                )
              }
        ...
        <div className="dp-footer">
          {!isAdd && (
            <button
              ...
              onMouseOver={(e) => {
                e.currentTarget.style.borderColor = "var(--p-red)";
              }}
              onMouseOut={(e) => {
                e.currentTarget.style.borderColor = "var(--p-border-t)";
              }}
          ...
          {isAdd ? (
            <>
              ...
              <button
                ...
                onMouseOver={(e) => {
                  e.currentTarget.style.borderColor = "var(--p-text-g)";
                }}
                onMouseOut={(e) => {
                  e.currentTarget.style.borderColor = "var(--p-border-t)";
                }}
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 500,
                  cursor: "pointer",
                  border: "1px solid var(--p-blue-700)",
                ...
                onMouseOver={(e) => {
                  e.currentTarget.style.background = "var(--p-blue-700)";
                }}
                onMouseOut={(e) => {
                  e.currentTarget.style.background = "var(--p-blue)";
                }}
              >
                Add Task
              </button>
      ...
      {showCommentSendConfirm && (
        ...
        >
          ...
          >
            ...
            >
              <Upload
                style={{ width: 20, height: 20, color: "var(--p-purple)" }}
              />
            </div>
            <h2
              style={{
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={() => {
                  setShowCommentSendConfirm(false);
                  void handleSubmitComment().then(() => onClose());
                }}
                style={{
                  flex: 1,
                  padding: "9px 14px",
...
function DpMetaRow({ task }: { task: Task }) {
    ...
    items.push(
      <span
        ...
        style={{
          ...
          gap: 4,
          padding: "2px 8px",
          background: "color-mix(in srgb, var(--p-purple) 14%, transparent)",
          border:
            "1px solid color-mix(in srgb, var(--p-purple) 36%, transparent)",
          borderRadius: 100,
          fontSize: 10,
          color: "var(--p-purple-300)",
          fontFamily: "monospace",
  ...
  const hasSource =
    task.file || (task.selector && task.selector !== "/") || task.cssSelector;
  if (hasSource) {
    ...
    const selectorText =
      task.cssSelector ??
      (task.selector && task.selector !== "/" ? task.selector : "");
    const sourceText = fileLine || selectorText;
    if (sourceText) {
      const display =
        sourceText.length > 40 ? `…${sourceText.slice(-40)}` : sourceText;
      const urlDisplay = task.url
        ? task.url.length > 30
          ? `…${task.url.slice(-30)}`
          : task.url
        : "";
      items.push(
        <span
          ...
          style={{
            ...
            overflow: "hidden",
            border:
              "1px solid color-mix(in srgb, var(--p-blue) 15%, transparent)",
            fontSize: 10,
            fontFamily: "monospace",
            whiteSpace: "nowrap",
          }}
        >
          <span
            title={[fileLine, selectorText].filter(Boolean).join(" · ")}
            style={{
              padding: "2px 6px",
          ...
          {urlDisplay && (
            <span
              ...
              style={{
                padding: "2px 6px",
                background:
                  "color-mix(in srgb, var(--p-border) 72%, transparent)",
                color: "var(--p-text-g)",
                borderLeft:
                  "1px solid color-mix(in srgb, var(--p-blue) 18%, transparent)",
              }}
            >
              {urlDisplay}
            </span>
    ...
    const urlDisplay =
      task.url.length > 40 ? `…${task.url.slice(-40)}` : task.url;
    items.push(
      <span key="url" className="dp-source-pill" title={task.url}>
        {urlDisplay}
      </span>,
    );
  }
    ...
    items.push(
      <span
        ...
        style={{
          ...
          border:
            "1px solid color-mix(in srgb, var(--p-purple) 24%, transparent)",
          borderRadius: 100,
          fontSize: 10,
          color: "var(--p-purple-300)",
        }}
