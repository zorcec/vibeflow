...
export function TaskDetailsTab({
  ...
  const fileList: Array<{
    ...
    ? liveFiles.map((f) => ({
        name: f.name,
        linkedPath: f.linkedPath,
        addedAt: f.createdAt,
        url: f.url,
      }))
    : taskFiles;
  const firstImage = fileList?.find((f) =>
  ...
  const [confirmDeleteScreenshot, setConfirmDeleteScreenshot] = React.useState<
    string | null
  >(null);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {/* Description */}
      <div>
        <div className="dp-meta-label" style={{ marginBottom: 4 }}>
          Description
        </div>
      ...
      {consolePart && <ConsoleLogsSection logs={consolePart} />}
        ...
        <div>
          ...
          >
            ...
            >
              ...
              >
                {firstImage.addedAt
                  ? `Captured ${formatDate(firstImage.addedAt)}`
                  : "Screenshot attached"}
              </span>
              <button
                onClick={() => onFilePreview(firstImage.name, screenshotUrl)}
      ...
      {task && (
        ...
        >
          <MetaTile label="Created" id="dp-created">
            {task.createdAt ? formatDate(task.createdAt) : "—"}
          </MetaTile>
          <MetaTile label="ID" id="dp-id">
            <span style={{ fontFamily: "monospace" }}>{task.id}</span>
          </MetaTile>
          <MetaTile label="Author" id="dp-author">
            {task.authorName ?? task.author ?? "—"}
          </MetaTile>
          {(task.commit || task.commits?.length) && (
            <CommitTile task={task} githubUrl={githubUrl} />
          )}
      ...
      <ConfirmModal
        ...
        confirmLabel="Remove"
        onConfirm={() => {
          if (confirmDeleteScreenshot)
            onDeleteScreenshot(confirmDeleteScreenshot);
...
function CommitTile({
  ...
  const allCommits = task.commits?.length
    ? task.commits
    : [
        {
          sha: task.commit!,
  ...
  const tooltipLines = allCommits
    .map(
      (c: { sha: string; message: string; timestamp: string }) =>
 ...
 */
function splitConsoleLogs(description: string): {
  userPart: string;
  consolePart: string | null;
} {
  const SEPARATOR = "\n---\n**Console logs**";
