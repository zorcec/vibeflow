...
export function FilesList({ files, loading, error, baseUrl, taskId, onPreview, onDelete }: Props) {
        ...
        <div style={{ marginTop: 4 }}>
          <button
            ...
            style={{
              ...
              color: 'var(--p-text-f)', fontSize: 11, fontWeight: 500,
            }}
          >
            <span style={{ fontSize: 10, transition: 'transform 0.15s', transform: systemOpen ? 'rotate(90deg)' : 'rotate(0deg)', display: 'inline-block' }}>▶</span>
            <span style={{
              background: 'var(--p-border)', borderRadius: 4, padding: '1px 5px',
          ...
          {systemOpen && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 }}>
              ...
              <p style={{ color: 'var(--p-text-f)', fontSize: 10, margin: '2px 0 0', fontStyle: 'italic' }}>
                DOM baselines & screenshots captured automatically — re-captured on every verify run, safe to ignore.
              </p>
            </div>
          )}
        </div>
