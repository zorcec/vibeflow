...
export function connectWS(): void {
  ...
  try {
    state.ws = new WebSocket(PROTO_CONFIG.wsUrl);
  } catch {
    scheduleReconnect();
    return;
  }
  ...
  state.ws.onmessage = (e: MessageEvent) => {
    ...
    if (msg.type === "task-changed" || msg.type === "task-deleted")
      state.onTasksUpdatedMessage?.();
  };
...
function scheduleReconnect(): void {
  const delay = Math.min(
    RECONNECT_BASE * 2 ** state.reconnectAttempt,
    RECONNECT_MAX,
  );
...
function checkHealth(): void {
  ...
  const healthUrl = PROTO_CONFIG.wsUrl
    .replace("ws://", "http://")
    .replace("wss://", "https://")
    .replace("/ws", "/api/health");
