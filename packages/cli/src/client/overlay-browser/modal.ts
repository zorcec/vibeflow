...
export function showEditModal(task: Task): void {
  ...
  const footerCenter = buildSourceRow({
    file: task.file,
    line: task.line,
    col: task.col,
  ...
  btnSave.addEventListener("click", () => {
    ...
    fetch(url, {
      ...
      .then((d: { success?: boolean }) => {
        if (d.success) fetchTasksAndRender();
      })
...
function fetchTasksAndRender(): void {
  fetchTasks();
}
...
function buildSourceRow(src: SourceOpts): HTMLElement {
  ...
  const sourceEl = el(
    "span",
    { className: "modal-source-label", title: src.file ?? "" },
    label,
  );
  if (src.file && src.line != null) {
    const link = el(
      "a",
      {
        ...
        href: `vscode://file${src.file}:${src.line}`,
        target: "_blank",
      },
      "↗",
    );
...
export function showInspectModal(
  element: Element,
  ...
  const classes = Array.from(element.classList).filter(
    (c) => !c.startsWith("vibeflow-"),
  );
  const dataAttrs = Array.from(element.attributes)
    .filter(
      (a) => a.name.startsWith("data-") && !a.name.startsWith("data-proto"),
    )
  ...
  function makeRow(label: string, value: string, copyable = true): HTMLElement {
    ...
    if (copyable && value) {
      const btn = el(
        "button",
        { className: "inspect-copy-btn", title: "Copy" },
        "⎘",
      );
  ...
  if (cssSelector !== pointer.selector)
    rows.appendChild(makeRow("CSS selector", cssSelector));
  if (pointer.test_id) rows.appendChild(makeRow("Test ID", pointer.test_id));
  if (elId) rows.appendChild(makeRow("id", elId));
  if (classes.length > 0)
    rows.appendChild(makeRow("Classes", classes.join(" ")));
    ...
    if (pointer.line != null) {
      const openLink = el(
        "a",
        {
          ...
          href: `vscode://file${pointer.file}:${pointer.line}`,
          target: "_blank",
        },
        "↗",
      );
  ...
  if (pointer.component)
    rows.appendChild(makeRow("Component", pointer.component));

  // data-* attributes
  for (const attr of dataAttrs)
    rows.appendChild(makeRow(attr.name, attr.value));

  // ── Copy all button ──────────────────────────────────────────────────────
  const copyAllBtn = el(
    "button",
    { className: "inspect-copy-all-btn" },
    "⎘ Copy all",
  );
  copyAllBtn.addEventListener("click", () => {
    ...
    if (pointer.file)
      lines.push(
        `Source: ${pointer.file}${pointer.line != null ? `:${pointer.line}` : ""}${pointer.col != null ? `:${pointer.col}` : ""}`,
      );
    ...
    if (cssSelector !== pointer.selector)
      lines.push(`CSS selector: ${cssSelector}`);
  ...
  const box = el(
    "div",
    { className: "vibeflow-inspect-modal" },
