...
export function toggleOverlaySidebar(): void {
  /* sidebar removed */
}
export function closeOverlaySidebar(): void {
  /* sidebar removed */
}
export function setOverlayTriggerAnnotating(_active: boolean): void {
...
function CornerTrigger({
  ...
  React.useEffect(() => {
    if (pos) {
      try {
        localStorage.setItem("vibeflow-trigger-pos", JSON.stringify(pos));
      } catch {
        /* ignore */
      }
    }
  ...
  const title = isDragging
    ? "Drag to reposition"
    : isHolding
      ? "Drag to reposition · Release to open"
      : "Open Kanban board · Hold to drag";
...
function OverlayAddModal({ opts, onClose, onSubmit }: AddModalProps) {
  ...
  function onHeaderPointerMove(e: React.PointerEvent<HTMLDivElement>) {
    ...
    const dy = e.clientY - dragOrigin.current.mouseY;
    setDragPos({
      x: dragOrigin.current.posX + dx,
      y: dragOrigin.current.posY + dy,
    });
  }
  ...
  const statusBtnBase: React.CSSProperties = {
    ...
    fontSize: 11,
    cursor: "pointer",
    border: "1px solid var(--vibeflow-border-strong)",
  ...
  return (
    <div
      className="vibeflow-modal-backdrop"
      onClick={handleBackdropClick}
      onKeyDown={handleKeyDown}
    >
      <div
        className="vibeflow-modal overlay-add-modal"
        role="dialog"
        aria-modal="true"
        style={modalStyle}
      >
        ...
        >
          ...
          <input
            ...
            style={{
              ...
              fontWeight: 600,
              padding: "4px 8px",
              border: `1px solid ${titleError ? "rgba(239,68,68,0.7)" : "transparent"}`,
              borderRadius: 6,
              background: titleError ? "rgba(239,68,68,0.07)" : "transparent",
              color: "var(--vibeflow-text-primary)",
              fontFamily: "inherit",
              outline: "none",
              cursor: "text",
            }}
            onFocus={(e) => {
              if (!titleError) {
                e.currentTarget.style.borderColor =
                  "var(--vibeflow-accent-strong)";
                e.currentTarget.style.background =
                  "var(--vibeflow-surface-elevated)";
              }
            }}
            onBlur={(e) => {
              if (!titleError) {
          ...
          <button
            ...
            onMouseOver={(e) => {
              e.currentTarget.style.background =
                "var(--vibeflow-surface-hover)";
              e.currentTarget.style.color = "var(--vibeflow-text-secondary)";
            }}
            onMouseOut={(e) => {
              e.currentTarget.style.background = "transparent";
              e.currentTarget.style.color = "var(--vibeflow-text-muted)";
            }}
            aria-label="Close"
          >
            ✕
        ...
        <div
          style={{
            ...
            gap: 6,
            padding: "8px 16px",
            borderBottom: "1px solid var(--vibeflow-border-strong)",
          }}
        >
          <span
            style={{
        ...
        {(opts.file || opts.component) && (
          <div
            style={{
              ...
              gap: 8,
              padding: "6px 16px",
              background: "var(--vibeflow-surface-elevated)",
              borderBottom: "1px solid var(--vibeflow-border-subtle)",
              fontSize: 11,
              color: "var(--vibeflow-text-muted)",
            }}
          >
            {opts.file && (
              <a
                ...
                style={{
                  ...
                  padding: "2px 6px",
                  borderRadius: 4,
                  background: "var(--vibeflow-accent-soft)",
                  border: "1px solid var(--vibeflow-accent-border)",
                  color: "var(--vibeflow-accent-strong)",
                  textDecoration: "none",
                  fontSize: 11,
                }}
              >
                🗎 {opts.file.split("/").slice(-2).join("/")}
                {opts.line == null ? "" : `:${opts.line}`}
              </a>
            )}
            {opts.component && (
              <span
        ...
        <div className="modal-tabs">
          <div
            className={`modal-tab${showPreview ? "" : " active"}`}
        ...
        {type === "Bug" && (
          ...
          >
            <span>🔴</span>
            <span>
              Console errors &amp; warnings will be attached to this report
            </span>
          </div>
        )}
...
export function OverlayApp({ onOpenKanban, onSubmitTask }: OverlayAppProps) {
  const [addModalOpts, setAddModalOpts] = React.useState<AddModalOpts | null>(
    null,
  );
  ...
  React.useEffect(() => {
    ...
    _externalHideTrigger = () => {
      try {
        localStorage.setItem(TRIGGER_HIDDEN_KEY, "1");
      } catch {
        /* ignore */
      }
      setIsHidden(true);
  ...
  const isKanbanPage =
    ...
    document.body.hasAttribute("data-vibeflow-cli-kanban");
