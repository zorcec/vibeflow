...
| `--priority` does not exist as a CLI flag at all | `tasks` option list has `--status --type --user --edit --add --title --set-status --description --json --commit --task --message --comment --commit-message --get --next --tag --report-file --branch --set-verify --verify-reason --limit --dry-run --fields` |
...
| 6 | No e2e tests for gate behaviour | **Scheduled** — Fix 1.3 spawn harness + Fix 2.5 parity tests; add `tests/e2e/review-gate.test.ts` covering: unverified UI task → blocked (error names `--set-verify pass|fail|cannot`); annotated UI task with `--set-verify pass` → allowed; `--set-verify cannot` without reason → rejected; non-UI task (no url/selector) → auto-allowed; MCP `update_task(review)` on unverified UI task → `E_VERIFY_REQUIRED`; setting `in-progress` clears `verified` |
